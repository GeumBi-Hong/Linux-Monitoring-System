...
def on_message(client, userdata, msg):
	...
	#FIND LASTEST DATA
        for doc in collection.find().sort([('_id', -1)]).limit(1):
            ...
            #print(socket.inet.aton(IP).hex()+cpu_num)
            client.publish("mon/mem/nom",socket.inet_aton(IP).hex()+nom_mem)
            client.publish("mon/mem/act",socket.inet_aton(IP).hex()+act_mem)
